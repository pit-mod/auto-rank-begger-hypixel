...
/**
 * Renders the HUD overlay with a high-fidelity modern dashboard aesthetic.
 */
public class HudRenderer {
    ...
    private void renderPolar(RenderGameOverlayEvent.Post event, RankBeggerModule mod) {
        ...
        // Cyan Outer Glow (Stable and targeted)
        for (int i = 1; i <= 8; i++) {
        ...
        // Footer: Logo + Polar v3.2.11
        float logoSize = 20; // Maximum visibility
        ...
        GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f); // Force 100% brightness
    ...
    private void renderPolinex(RenderGameOverlayEvent.Post event, RankBeggerModule mod) {
        ...
        float x = (float) mod.hudX.getValue();
        float y = (float) mod.hudY.getValue();
        float width = 140; // Narrower for a more vertical look
        ...
        // Master Container (Symmetrical 4px padding on all sides)
        RenderUtil.drawRoundedRect(x - 4, y - 4, width + 8, totalH + 4, 6, 0xB010162B);
        // Outer outline removed
        ...
        for (int i = 0; i < sections.size(); i++) {
            ...
            if (!isFirst) {
                for (PolinexField f : sec.fields) {
                    if (isLast) {
                        ...
                        // Left Aligned (for wrapped lines)
            ...
            currentY += secH + 4; // More vertical gap between boxes
        }
    ...
    private void renderTaunahi(RenderGameOverlayEvent.Post event, RankBeggerModule mod) {
        ...
        for (HudLine hl : lines) {
            if (hl.isSeparator) {
                height += 8; // More space for separator
            } else {
    ...
    private void renderNebula(RenderGameOverlayEvent.Post event, RankBeggerModule mod) {
        ...
        String lastMsg = ctx.lastSentMessage != null ? ctx.lastSentMessage : "None";
        // Calculate wrap width considering the 0.85f scale we will render it at (115 / 0.85 = ~135)
        ...
        // 1. Top-Notch Layered Gaussian Glow
        // Stacking faint solid rectangles creates a perfect exponential alpha falloff (Gaussian blur)
        for (int i = 8; i >= 1; i--) {
            int alpha = 10 + (8 - i) * 3; 
        ...
        // 2. Main Background Plate (Covers the center of the glow layers)
        drawRoundedRect(x, y, width, height, radius, 0xE6111111);

        // 3. Crisp Inner Outline
        drawRoundedRectOutline(x, y, width, height, radius, 1.0f, 0x4D8A5CF5); // 30% purple edge

        // 4. Header Branding (Icon & Title)
        float iconCx = x + 28;
        float iconCy = y + 28;

        drawSolidCircle(iconCx, iconCy, 14, 0x33000000); // Faint dark circle
        ...
        drawProgressCircle(iconCx, iconCy, 14, 2.0f, 1.0f, 0x338A5CF5); // Background track
        drawProgressCircle(iconCx, iconCy, 14, 2.0f, progress, 0xFF8A5CF5); // Active track
    ...
    private void drawSolidCircle(float cx, float cy, float radius, int color) {
        ...
        GL11.glVertex2d(cx, cy); // Center point
    ...
    private void renderWielix(RenderGameOverlayEvent.Post event, RankBeggerModule mod) {
        ...
        float x = (float) mod.hudX.getValue();
        float y = (float) mod.hudY.getValue();
        float width = 240; // Authentic Shellwise width
        ...
        for (String line : wrappedLines) {
            fontSmall.drawString(line, x + 20, currY, 0xFFC8DCFF); // Shellwise VALUE_COLOR
            currY += fontSmall.getFontHeight() + 4;
        }
        ...
        fontSmall.drawString("Wielix", x + 16 + logoS + 4, footerY + 1, 0xFFC8A03C); // Shellwise FOOTER_LEFT tint
        fontSmall.drawString(" 0.10.0", x + 16 + logoS + 4 + fontSmall.getStringWidth("Wielix"), footerY + 1, 0xFF777777);
        
        String sessionTime = formatSessionTime(now - ctx.sessionStartTime);
        fontSmall.drawString(sessionTime, x + width - 16 - fontSmall.getStringWidth(sessionTime), footerY + 1, 0xFF64A0E6); // Shellwise FOOTER_RIGHT tint
